...
public class RentalController {
    ...
    @Operation(summary = "Creates a new rental", description = "Creates a new rental with the provided inputs", responses = {
            @ApiResponse(responseCode = "200", description = "Rental created", content = @Content(schema = @Schema(implementation = MessageResponseDTO.class))),
    ...
    @Operation(summary = "Updates a rental", description = "Updates a rental with the provided inputs", responses = {
            @ApiResponse(responseCode = "200", description = "Rental updated", content = @Content(schema = @Schema(implementation = MessageResponseDTO.class))),
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content()),
            @ApiResponse(responseCode = "401", description = "User is unauthorized", content = @Content())
    }, security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @GetMapping
    @Operation(summary = "Get all rentals", description = "Retrieves details of all rentals", responses = {
            @ApiResponse(responseCode = "200", description = "All rentals returned", content = @Content(schema = @Schema(implementation = RentalListResponseDTO.class))),
            @ApiResponse(responseCode = "401", description = "User is unauthorized", content = @Content())
    }, security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @Operation(summary = "Get picture of a rental", description = "Returns the picture of a rental", responses = {
    ...
    public ResponseEntity<Resource> servePicture(
            @Parameter(description = "Name of the file for the rental picture.") @PathVariable String fileName) {
